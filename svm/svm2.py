...
# For use with test / training datasets : isolet<....>.{train | test}
...
# load training data set (N.B. delimiter is comma)

reader=csv.reader(open("isolet1+2+3+4.train","rt", encoding='ascii'),delimiter=',')

attribute_list = []
label_list = []

for row in reader:
        # attributes in columns 0-616, class label in column 617
        attribute_list.append(list(row[i] for i in (range(0,617))))
        label_list.append(row[617])

training_attributes=np.array(attribute_list).astype(np.float32)
training_class_labels=np.array(label_list).astype(np.float32)

# load testing data set (N.B. delimiter is comma)

reader=csv.reader(open("isolet5.test","rt", encoding='ascii'),delimiter=',')
...
for i in range(0, len(testing_attributes[:,0])) :
    ...
    print("Test data example : {} : result =  {}".format((i+1), chr(ord('A') -1 + int(result[0]))));
