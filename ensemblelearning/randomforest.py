...
parameters = dict(max_categories = 5,      # max number of categories (use sub-optimal algorithm for larger numbers)
                  max_depth = 5,            # max tree depth
                  min_sample_count = 5,     # min sample count
                  cv_folds = 1,            # the number of cross-validation folds/iterations
                  ...
                  nactive_vars = 5, # number of randomly selected subset of attributes used at each tree node to find the best split(s).
                  max_num_of_trees_in_the_forest = 300, # typically the more trees you have the better the accuracy.
                  forest_accuracy = 0.001,  # sufficient accuracy on task
                  term_crit = (cv2.TERM_CRITERIA_COUNT|cv2.TERM_CRITERIA_EPS, 3000, 0.001) # termination criteria (iteration / accuracy)
                  #term_crit = (cv2.TERM_CRITERIA_EPS, 300, 0.001),
                  )
...
dforest = cv2.RTrees()
dforest.train(training_attributes, cv2.CV_ROW_SAMPLE, training_class_labels, None, None, var_types, None, params=parameters)
...
for i in range(0, len(testing_attributes[:,0])) :
    ...
    result = dforest.predict(testing_attributes[i,:])
